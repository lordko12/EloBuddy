...
{
    public class Orbwalk
    {
        static Menu menu;

        static float lastaa = 1f;
        ...
        {
            menu = MainMenu.AddMenu("FunboxOrb", "funboxorbfh");
            menu.AddSeparator();
            ...
            menu.AddSeparator();
            menu.Add("kite", new Slider("Kite: MINUS[performance] PLUS[humanizer or if you have stutter]", 0, -50, 50));
            menu.AddSeparator();
            ...
            {
              Game.OnUpdate += Game_OnUpdate_Graves;
            }
            else if (ObjectManager.Player.ChampionName == "Jhin")
            {
              Game.OnUpdate += Game_OnUpdate_Jhin;
            }
            else
            {
            ...
            {
                GameObject.OnCreate += GameObject_OnCreate_Garen;
                GameObject.OnDelete += GameObject_OnDelete_Garen;
            }
            else if (ObjectManager.Player.ChampionName == "TwistedFate")
            {
                Obj_AI_Base.OnProcessSpellCast += Obj_AI_Base_OnProcessSpellCast_TF;
            }
        ...
        private static void Obj_AI_Base_OnProcessSpellCast_TF(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (sender.IsMe && (args.SData.Name == "GoldCardPreAttack" || args.SData.Name == "RedCardPreAttack" || args.SData.Name == "BlueCardPreAttack"))
            {
                lastaa = Game.Time * 1000;
            }
        }

        static void GameObject_OnDelete_Garen(GameObject sender, EventArgs args)
        {
            if (sender.Name == "Garen_Base_E_Spin.troy")
            {
                orbattack = true;
            }
            if (sender.Name == "Garen_Steel_Q_Electric_Sword.troy")
            {
                lastaa = Game.Time * 1000;
            }
        }

        static void GameObject_OnCreate_Garen(GameObject sender, EventArgs args)
        {
            if (sender.Name == "Garen_Base_E_Spin.troy")
            {
                orbattack = false;
        ...
        static void Game_OnUpdate_Graves(EventArgs args)
        {
        ...
        static void Game_OnUpdate_Jhin(EventArgs args)
        {
            if (menu["orb"].Cast<CheckBox>().CurrentValue)
            {
                ResetDelay(resetdelay);
        ...
        static void Game_OnUpdate(EventArgs args)
        {
            if (menu["orb"].Cast<CheckBox>().CurrentValue)
            {
                ...
                if (Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.Combo))
                {
                    ...
                    if (Target != null)
                    {
                        if (orbattack && Game.Time * 1000 > lastaa + ObjectManager.Player.AttackDelay * 1000 - Game.Ping * 2.15)
                        {
                            Player.IssueOrder(GameObjectOrder.AttackUnit, Target);
                        }
                        else if (orbmove && Game.Time * 1000 > lastmove + menu["humanizer"].Cast<Slider>().CurrentValue && Game.Time * 1000 > lastaa + ObjectManager.Player.AttackCastDelay * 1000 - Game.Ping / 2.15 + (35f - cpuspeed) + menu["kite"].Cast<Slider>().CurrentValue)
